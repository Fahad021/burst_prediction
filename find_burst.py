...
def check_slope(arr, start, end, point, st_p):
    ...
    angle = np.std(arr[st_p:]) / win_size
...
for i in xrange(0, m):
    ...
    # if t_localmax[0] < t_localmin[0], add start point as the first local_min or delete first local_max
    if local_max[0] < local_min[0]:
        if local_max[0] == st_p:
            local_max = local_max[1:]
        else:
            local_min = [st_p] + local_min

    # if t_localmax[len] > t_localmin[len], add end time as the last local_min (put an end to the burst)
    if local_max[len(local_max)-1] > local_min[len(local_min)-1]:
        local_min = local_min + [n-1]

    j = 0
    while j < len(local_min) - 1:
        if j >= len(local_max):
            break

        burst_end = local_min[j + 1]

        if check_slope(series[i], local_min[j], burst_end, local_max[j], st_p):
            start, end, top = local_min[j], burst_end, local_max[j]
            # print start, end, top
            while True:
                # if last burst, no need to combine
                if j >= len(local_min) - 2 or j >= len(local_max) - 2:
                    break

                end2 = local_min[j + 2]

                check_sum = combine_bursts(series[i], top, local_max[j+1], start, end, end2)
                # print check_sum
