    ...
    while j < len(local_min):
        ...
        if j < len(local_min) - 1:
            burst_end = min(local_min[j + 1], 2 *
                            local_max[j] - local_min[j] + 30)
        else:
            burst_end = min(n-1, 2 * local_max[j] - local_min[j] + 30)
        # if j < len(local_min) - 1:
        ...
        if check_slope(series[i], local_min[j], burst_end, local_max[j], st_p):
            ...
            while True:
                ...
                if j >= len(local_min) - 1 or j >= len(local_max) - 1:
                    break

                if j == len(local_min) - 2:
                    end2 = min(n-1, 2 * local_max[j+1] - local_min[j+1])
                else:
                    end2 = local_min[j + 2]
